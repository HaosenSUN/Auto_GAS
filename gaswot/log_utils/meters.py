...
class RecorderMeter(object):
    ...
    def plot_curve(self, save_path):
        ...
        for i, (label, arr) in enumerate(zip(labels, arrs)):
            la_mode, la_type = label.split('_')[0], '_'.join(
                label.split('_')[1:])
            color, linestyle = colors[label_modes.index(la_mode)], linestyles[
                label_types.index(la_type)]
            plt.plot(
                x, arr, color=color, linestyle=linestyle, label=label, lw=2)
