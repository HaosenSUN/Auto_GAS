...
from gaswot.tnb101.models.model_info import get_inference_time, get_params
...
class ModelDB(object):
    ...
    def __init__(self, cfg, encoder_str, logger, world_size):
        ...
        self.encoder_ckpt_path = str(
            Path(cfg['log_dir']) /
            f'checkpoints/{self.encoder_str}_encoder.pth')
        self.decoder_ckpt_path = str(
            Path(cfg['log_dir']) /
            f'checkpoints/{self.encoder_str}_decoder.pth')
        if 'discriminator' in self.cfg.keys():
            self.discriminator_ckpt_path = str(
                Path(cfg['log_dir']) /
                f'checkpoints/{self.encoder_str}_discriminator.pth')
    ...
    def save(self, model_dic, epoch, metrics):
        ...
        for category, model in model_dic.items():
            torch.save(
                ...
                str(
                    ...
                    f'checkpoints/{self.encoder_str}_{category}_{epoch}.pth'))
            torch.save(
                model.state_dict(),
                str(
                    Path(self.cfg['log_dir']) /
                    f'checkpoints/{self.encoder_str}_{category}_final.pth'))

        # Save metrics
        metric_collection = {}
        assert isinstance(metrics, dict)
        assert len(metrics.keys()) == len(
            self.cfg['metric_content']), f'Received metrics {metrics.keys()}'
        ...
        model_db[
            self.encoder_str]['metrics'][f'epoch_{epoch}'] = metric_collection
        save_json(model_db, self.model_db_filename)

    def change_status_save(self,
                           status):  # ['running', 'finished', 'corrupted']
    ...
    def create_initial_dic(self):
        initial_version_dic = {
            ...
            'decoder_ckpt_file':
            self.decoder_ckpt_path,
            'total_params':
            get_params(self.cfg['encoder']),
            'inference_time':
            get_inference_time(self.cfg['model_type'], (1, 3, 720, 1080)),
            'metrics': {}
        }
        if 'discriminator' in self.cfg.keys():
            initial_version_dic[
                'discriminator_ckpt_path'] = self.discriminator_ckpt_path
        return initial_version_dic

    def _initialize_dict(self):
        if not os.path.exists(self.model_db_filename):
            initial_dict = {self.encoder_str: self.create_initial_dic()}
